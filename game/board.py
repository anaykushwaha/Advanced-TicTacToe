# board.py 
# Defines the Board class, which manages the game, 
...
from game.helper import create_empty_board, get_available_moves 
...
class Board: 
    ...
    def place_move(self, row: int, col: int, symbol: str) -> bool: 
        ...
        if symbol not in (PLAYER_X, PLAYER_O):
            return False
    ...
    def check_winner(self) -> Optional[str]: 
        ...
        for row in self.board:
            if (
                row[0] != EMPTY_CELL
                and row.count(row[0]) == BOARD_SIZE
            ):
                return row[0]
        ...
        # Secondary diagonal
        symbol = self.board[0][BOARD_SIZE - 1]

        if (
            symbol != EMPTY_CELL
            and all(
                self.board[i][BOARD_SIZE - 1 - i] == symbol
                for i in range(BOARD_SIZE)
            )
        ):
            return symbol
